...
export const generateEmptyLayout = () => {
  ...
  for (let i = 0; i <= 9; i++) {
    for (let j = 0; j <= 9; j++) {
      ...
      result.push({
        value: `${numberChar.toString()}${letterCharacter}`,
        cellStatus: CELL_STATE.empty,
      });
    }
  }
  return result;
...
const generateShip = (isShipPlaced, cells, cellNumbers) => {
  ...
  const direction =
    generateRandomNumber(0, 99) % 2 === 0 ? "horizontal" : "vertical";
  ...
  if (direction === "horizontal") {
    ...
    const shipCells = cells.filter(
      (cell, indexOfCell) =>
        indexOfCell >= point && indexOfCell < point + cellNumbers
    );

    const arePointsUnavailable =
      shipCells.filter((item) => item.cellStatus !== CELL_STATE.empty).length >
      0;
